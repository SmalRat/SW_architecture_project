...
def reserve_table(table_id, time, user):
    data = {"user_name": user, "table_number": table_id, "booking_time": time}
    ...
    for idx, table in enumerate(tables_resp):
        if tables[table["_id"]]:
            ...
            if available_times:
                ...
                num_rows = (
                    len(available_times) + cols_per_row - 1
                ) // cols_per_row  # Calculate needed rows
                ...
                if (
                    ...
                    and not st.session_state.form_submitted
                ):
                    selected_time = st.session_state.selected_time
                    st.write(
                        f"You selected {selected_time}. Please enter your details to confirm."
                    )
                    ...
                    with st.form(key="user_details_form"):
                        ...
                        if submit_button:
                            ...
                            if reserve_table(
                                table["_id"], selected_time.split(":")[0], name
                            ):
                                st.success(
                                    f"Reservation confirmed for {name} at {selected_time}"
                                )
                                sleep(2)
